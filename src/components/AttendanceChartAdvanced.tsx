...
import { BarChart, Bar, LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, PieChart, Pie, Cell, type PieLabelRenderProps } from 'recharts';
...
export const AttendanceChartAdvanced = ({ attendance }: AttendanceChartAdvancedProps) => {
    ...
    const totalValue = useMemo(() => pieData.reduce((sum, entry) => sum + entry.value, 0), [pieData]);

    const renderLabel = ({ name, value }: PieLabelRenderProps): string => {
        const percent = totalValue > 0 ? ((value as number) / totalValue) * 100 : 0;
